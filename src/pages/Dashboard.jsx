      ...
      <motion.div variants={fadeUp}>
        <Card variant="default" padding="md" className="dash-map-card">
          ...
          {allBillboards.length > 0 && (
            <div className="dash-qr-locations">
              <p className="dash-qr-locations-title">QR Code Locations</p>
              <div className="dash-qr-locations-grid">
                {allBillboards.map(b => (
                  <div key={b.id} className={`dash-qr-loc-item ${!b.isActive ? 'dash-qr-loc-item--inactive' : ''}`}>
                    <span className={`dash-qr-loc-dot ${b.isActive ? 'dash-qr-loc-dot--active' : ''}`} />
                    <div className="dash-qr-loc-info">
                      <span className="dash-qr-loc-name">{b.locationName}</span>
                      <span className="dash-qr-loc-meta">
                        <MapPin size={10} />
                        {b.city || '—'}
                        {b.coordinates?.lat && (
                          <>
                            <Navigation size={10} style={{ marginLeft: 6 }} />
                            {Number(b.coordinates.lat).toFixed(4)}, {Number(b.coordinates.lng).toFixed(4)}
                          </>
                        )}
                      </span>
                    </div>
                    <span className="dash-qr-loc-scans">{(b.totalScans ?? 0).toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>

...
export default function Dashboard() {
  ...
  const billboardMap = allBillboards.reduce((acc, b) => { acc[b.id] = b; return acc }, {})

  function handleExport() {
    const rows = liveScans.map(s => {
    ...
    exportToCsv(`dashboard-export-${format(new Date(), 'yyyy-MM-dd')}.csv`, rows)
  }
    ...
    >
      ...
      <motion.div className="dash-header" variants={fadeUp}>
        ...
        <Button
          ...
          disabled={scansLoading || liveScans.length === 0}
        >
          Export Report
        </Button>
      </motion.div>

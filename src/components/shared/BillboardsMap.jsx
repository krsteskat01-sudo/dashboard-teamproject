...
function markerSize(scans, mini = false) {
  const base = mini ? 24 : 32
  const max  = mini ? 40 : 56
  if (!scans || scans === 0) return base
  return Math.min(base + Math.log10(scans + 1) * (mini ? 5 : 8), max)
}

function createBillboardIcon(billboard, mini = false) {
  const size  = markerSize(billboard.totalScans ?? 0, mini)
  ...
  const html = `
    <div style="
      ...
      font-size: ${size < 32 ? 9 : size < 40 ? 10 : 12}px;
      font-weight: 700;
      color: #fff;
      cursor: pointer;
          ...
          >
            ...
            {withCoords.map((board) => (
              <Marker
                ...
                icon={createBillboardIcon(board, mini)}
              >
                <Popup className="bmap-popup" closeButton={!mini}>
                  {mini ? (
                    ...
                    <div className="bmap-popup-content">
                      ...
                      <p className="bmap-popup-coords">
                        {Number(board.coordinates.lat).toFixed(6)}, {Number(board.coordinates.lng).toFixed(6)}
                      </p>
                      {onMarkerClick && (
                        <button
